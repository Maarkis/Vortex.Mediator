...
public sealed class MediatorGenerator : IIncrementalGenerator
{
    ...
    private static string RenderSource(GenerationModel model)
    {
        ...
        foreach (var request in model.Requests)
        {
            ...
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        return Invoke(behaviors.Count - 1);");
            builder.AppendLine();
            builder.AppendLine("        Task<" + request.ResponseType + "> Invoke(int index)");
            builder.AppendLine("        {");
            ...
            builder.AppendLine("            }");
            builder.AppendLine();
            builder.AppendLine("            return behaviors[index].Handle(request, cancellationToken, () => Invoke(index - 1));");
            builder.AppendLine("        }");
            builder.AppendLine("    }");
            builder.AppendLine();
        }
